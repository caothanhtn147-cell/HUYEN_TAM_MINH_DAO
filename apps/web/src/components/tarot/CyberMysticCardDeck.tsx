...
export const CyberMysticCardDeck: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-6 rounded-3xl bg-zinc-950/60 border border-amber-500/20 backdrop-blur-2xl shadow-2xl shadow-purple-950/30">
      <div className="flex items-center justify-between mb-8 pb-4 border-b border-amber-500/10">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-1">
            Chạm vào lá bài để mở hiệu ứng lật 3D & nhận thông điệp minh triết
          </p>
        </div>
        <div className="flex items-center gap-2 text-xs text-emerald-400 bg-emerald-950/40 px-3 py-1.5 rounded-full border border-emerald-500/20">
          <ShieldCheck className="h-3.5 w-3.5" /> Hardware Entropy Ready
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {SAMPLE_CARDS.map((card, idx) => {
          ...
          return (
            ...
            >
              <div
                className={`relative h-full w-full rounded-2xl transition-transform duration-700 transform-style-3d shadow-xl shadow-amber-950/20 ${
                  isFlipped ? "rotate-y-180" : "group-hover:scale-[1.02] group-hover:-translate-y-1"
                }`}
              >
                {/* CARD BACK (Mặt Sau 3D) */}
                <div className="absolute inset-0 h-full w-full rounded-2xl bg-gradient-to-br from-zinc-900 via-purple-950 to-zinc-950 p-4 border border-amber-500/30 flex flex-col items-center justify-between backface-hidden overflow-hidden">
                  ...
                  <div className="w-full flex justify-between items-center text-[10px] text-amber-400/60 font-mono">
                    <span>CYBER-MYSTIC</span>
                    <span>78 ARCANA</span>
                  </div>
                  <div className="my-auto flex flex-col items-center gap-2 text-center">
                    <div className="h-16 w-16 rounded-full border-2 border-amber-400/40 flex items-center justify-center bg-amber-500/10 shadow-lg shadow-amber-500/10">
                      <Sparkles className="h-8 w-8 text-amber-300 animate-pulse" />
                    </div>
                    <span className="text-xs font-semibold text-amber-200/80">Lật Bài Giải Quẻ</span>
                  </div>
                  <div className="w-full text-center text-[9px] text-zinc-500 tracking-widest uppercase">
                    HUYỀN TÂM MINH ĐẠO
                  </div>
                </div>

                {/* CARD FRONT (Mặt Trước 3D) */}
                <div className="absolute inset-0 h-full w-full rounded-2xl bg-gradient-to-br from-zinc-900 via-zinc-950 to-purple-950 p-5 border-2 border-amber-400/50 flex flex-col justify-between rotate-y-180 backface-hidden shadow-2xl shadow-amber-500/20">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {card.keywords.map((kw, kIdx) => (
                      <span
                        key={kIdx}
                        className="text-[10px] bg-amber-500/10 text-amber-300 px-2 py-0.5 rounded-full border border-amber-500/20"
                      >
                        #{kw}
                      </span>
                    ))}
      ...
      {activeCard && (
        <div className="mt-6 p-4 rounded-xl bg-purple-950/30 border border-purple-500/30 animate-fadeIn flex items-center justify-between">
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-lg bg-zinc-900 hover:bg-zinc-800 text-xs text-zinc-300 flex items-center gap-1 border border-zinc-700"
          >
