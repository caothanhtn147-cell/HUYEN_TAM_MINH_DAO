...
CONTEXT_BIRTH_DATE: Final[bytes] = b"huyentam:user_profile:birth_date:v1"
CONTEXT_BIRTH_TIME: Final[bytes] = b"huyentam:user_profile:birth_time:v1"
CONTEXT_BIRTH_LOCATION: Final[bytes] = b"huyentam:user_profile:birth_location:v1"
...
class UserProfile(Base):
    ...
    def get_birth_date(self, key: bytes | None = None) -> str | None:
        ...
        return decrypt_sensitive_value(
            self.birth_date_encrypted, context=CONTEXT_BIRTH_DATE, key=key
        )

    def set_birth_date(self, value: str | None, key: bytes | None = None) -> None:
        """Encrypt plaintext birth date string into birth_date_encrypted."""
        self.birth_date_encrypted = encrypt_sensitive_value(
            value, context=CONTEXT_BIRTH_DATE, key=key
        )

    def get_birth_time(self, key: bytes | None = None) -> str | None:
        """Decrypt birth_time_encrypted back to plaintext string."""
        return decrypt_sensitive_value(
            self.birth_time_encrypted, context=CONTEXT_BIRTH_TIME, key=key
        )

    def set_birth_time(self, value: str | None, key: bytes | None = None) -> None:
        """Encrypt plaintext birth time string into birth_time_encrypted."""
        self.birth_time_encrypted = encrypt_sensitive_value(
            value, context=CONTEXT_BIRTH_TIME, key=key
        )

    def get_birth_location(self, key: bytes | None = None) -> str | None:
        """Decrypt birth_location_encrypted back to plaintext string."""
        return decrypt_sensitive_value(
            self.birth_location_encrypted, context=CONTEXT_BIRTH_LOCATION, key=key
        )

    def set_birth_location(self, value: str | None, key: bytes | None = None) -> None:
        """Encrypt plaintext birth location string into ciphertext column."""
        self.birth_location_encrypted = encrypt_sensitive_value(
            value, context=CONTEXT_BIRTH_LOCATION, key=key
        )
